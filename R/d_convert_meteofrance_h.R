#' @title Convert a Meteo-France csv hourly data file into a htsr sqlite base
#'
#' @author P. Chevallier - dec 2023
#'
#' @description Convert a meteo-france csv data file into a htsr sqlite base.
...
# function d_convert_meteofrance_h
	d_convert_meteofrance_h <- function(fmeteo) {
		...
		NUM_POSTE<-NOM_USUEL<-LAT<-LON<-ALTI<- AAAAMMJJHH<- RR1<- FF<- DD<- FXY<- DXY<-
		FXI<- DXI<- FF2<- DD2<- FXI2<- DXI2<- FXI3S<- DXI3S<- T<- TD<- TN<- TX<- U<- UN<- UX<-
		TSV<- PMER<-PSTAT<- HNEIGEF<- NEIGETOT <- NULL
		...
		x <- select(x, NUM_POSTE, NOM_USUEL, LAT, LON, ALTI, AAAAMMJJHH, RR1, FF, DD, FXY, DXY,
								FXI, DXI, FF2, DD2, FXI2, DXI2, FXI3S, DXI3S, T, TD, TN, TX, U, UN, UX,
								TSV, PMER,PSTAT, HNEIGEF, NEIGETOT)

		# traitement datetime
		x$AAAAMMJJHH <- force_tz(ymd_h(paste0(str_sub(x$AAAAMMJJHH, 1, 4),"-",
																					str_sub(x$AAAAMMJJHH, 5, 6),"-",
																					str_sub(x$AAAAMMJJHH, 7, 8)," ",
																					str_sub(x$AAAAMMJJHH, 9, 10))),"CET")

		# type de donnees
		x$NUM_POSTE <- as.factor(x$NUM_POSTE)
		x$NOM_USUEL <- as.factor(x$NOM_USUEL)
		x$LAT <- as.numeric(x$LAT)
		x$LON <- as.numeric(x$LON)
		x$ALTI <- as.numeric(x$ALTI)
		x$RR1 <- as.numeric(x$RR1)
		x$FF <- as.numeric(x$FF)
		x$DD <- as.numeric(x$DD)
		x$FXY <- as.numeric(x$FXY)
		x$DXY <- as.numeric(x$DXY)
		x$FXI <- as.numeric(x$FXI)
		x$DXI <- as.numeric(x$DXI)
		x$FF2 <- as.numeric(x$FF2)
		x$DD2 <- as.numeric(x$DD2)
		x$FXI2 <- as.numeric(x$DXI2)
		x$DXI2 <- as.numeric(x$DXI2)
		x$FXI3S <- as.numeric(x$FXI3S)
		x$DXI3S <- as.numeric(x$DXI3S)
		x$T <- as.numeric(x$T)
		x$TD <- as.numeric(x$TN)
		x$TX <- as.numeric(x$TX)
		x$U <- as.numeric(x$U)
		x$UN <- as.numeric(x$UN)
		x$UX <- as.numeric(x$UX)
		x$TSV <- as.numeric(x$TSV)
		x$PMER <- as.numeric(x$PMER)
		x$PSTAT <- as.numeric(x$PSTAT)
		x$HNEIGEF <- as.numeric(x$HNEIGEF)
		x$NEIGETOT <- as.numeric(x$NEIGETOT)

		# identification stations
		cod_sta <- levels(x$NUM_POSTE)
		nom_sta <- levels(x$NOM_USUEL)
		coord_sta <- levels(as.factor(paste(as.character(x$LAT), as.character(x$LON),
									as.character(x$ALTI))))
		y <- str_split_fixed(coord_sta," ",3)

		# creation stations
		conn <- dbConnect(SQLite(),fsq)
		selection <- paste ("SELECT Id_Station FROM ST")
		table_sta <-dbGetQuery(conn, selection)
		list_sta <- table_sta$Id_Station
		dbDisconnect(conn)
		for (i in 1:length(cod_sta)){
			if(!(cod_sta[i] %in% list_sta))
				d_station (fsq, op = "C", sta = cod_sta[i], ty_st = "M",
									 name_st = nom_sta[i],
									 name_fld= c("Latitude", "Longitude", "Altitude", "Pays"),
									 value_fld= c(y[i,1], y[i,2], y[i,3], "France"),
									 bku = TRUE)
		}
		conn <- dbConnect(SQLite(),fsq)
		selection <- paste ("SELECT Id_Station FROM ST")
		table_sta <-dbGetQuery(conn, selection)
		cod_sta <- table_sta$Id_Station
		dbDisconnect(conn)
		...
		for (i in 1:length(cod_sta)) map(l, function(.x) d_sensor(fsq, op = "C", sta = cod_sta[i], sen=.x,
																	 table = "PR", bku = FALSE))
		xx <- transmute(x,Type_Station="M",Id_Station=NUM_POSTE, Capteur="RR1", Date = AAAAMMJJHH, Valeur = RR1)
		conn <- dbConnect(SQLite(),fsq)
		dbWriteTable(conn, name="PR", xx, append = TRUE)
		dbDisconnect(conn)

		# autres cas
		l <- as.vector(c("FF", "DD", "FXY", "DXY", "FXI", "DXI", "FF2", "DD2", "FXI2", "DXI2", "FXI3S",
					 "DXI3S", "T", "TD", "TN", "TX", "U", "UN", "UX", "TSV", "PMER","PSTAT", "HNEIGEF", "NEIGETOT"))
		for (i in 1:length(cod_sta)) map(l, function(.x) d_sensor(fsq, op = "C", sta = cod_sta[i], sen=.x,
																 table = "WE", bku = FALSE))
		for (j in 1:length (l)) {

			xx <- transmute(x,Type_Station="M",Id_Station=NUM_POSTE, Capteur=l[j], Date = AAAAMMJJHH, x[,7+j])
